  ...
  try {
    const result = await pool.query(`
      SELECT w."ID" as id, w."Name", w."Status", w."TrainingHours", w."FlightHours",
             w."WCI_Level", w."Competence", w."MpiID", w."CertificationExpirationDays", w."IsCritical",
...
router.post('/workstations', async (req, res) => {
  const { Name, Status, TrainingHours, FlightHours, WCI_Level, Competence, MpiID, CertificationExpirationDays, IsCritical } = req.body
  try {
    await pool.query(
      'INSERT INTO "Workstation" ("Name", "Status", "TrainingHours", "FlightHours", "WCI_Level", "Competence", "MpiID", "CertificationExpirationDays", "IsCritical") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
      [Name, Status ?? true, TrainingHours || 0, FlightHours || 0, WCI_Level || 1, Competence || null, MpiID || null, CertificationExpirationDays ?? null, IsCritical ?? false]
    )
    res.json({ success: true })
...
router.put('/workstations/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE "Workstation" SET "Name"=$1, "Status"=$2, "TrainingHours"=$3, "FlightHours"=$4, "WCI_Level"=$5, "Competence"=$6, "MpiID"=$7, "CertificationExpirationDays"=$8, "IsCritical"=$9 WHERE "ID"=$10',
      [Name, Status, TrainingHours || 0, FlightHours || 0, WCI_Level || 1, Competence || null, MpiID || null, CertificationExpirationDays ?? null, IsCritical ?? false, req.params.id]
    )
    res.json({ success: true })
