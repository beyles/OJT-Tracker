...
export default function Certifications() {
  ...
  useEffect(() => { loadCerts() }, [page, search])
  ...
  const canSave = form.employeeId && form.workstationId && form.result && form.date && !saving
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <input
              ...
              placeholder="Search by employee or workstation…"
            ...
            {canCreate && (
              ...
              >
                <span style={{ fontSize: '17px', lineHeight: 1 }}>+</span> New Certification
              </button>
            )}
          </div>
          ...
          <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '-6px' }}>
            {total > 0
              ? `${total.toLocaleString()} certification${total !== 1 ? 's' : ''}${search ? ` matching "${search}"` : ''}`
              : search ? `No certifications matching "${search}"` : 'No certifications recorded yet.'
            }
          </div>
            ...
            {certs.map(cert => (
              <div key={cert.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: '#111827' }}>{cert.EmployeeName}</div>
                ...
                <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>{cert.WorkstationName}</div>
                <div style={{ fontSize: '11px', color: '#9ca3af' }}>
                  {formatDate(cert.Date)}
                  {cert.CreatedByName && <span style={{ color: '#d1d5db' }}> · </span>}
                  {cert.CreatedByName && <span>by {cert.CreatedByName}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        {panelOpen && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#111827' }}>New Certification</div>
              <button
                onClick={closePanel}
                style={{
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {['Pass', 'Fail'].map(r => {
                    ...
                    const color = r === 'Pass' ? '#00c896' : '#ef4444'
                    return (
                      <button
                        key={r}
